var myProductName = "davehttp", myVersion = "0.5.2";    
...
const urlDefaultFavicon = "//s3.amazonaws.com/scripting.com/favicon.ico"; //11/24/18 by DW & 1/30/23 by DW && 2/3/23 by DW
...
function startup (config, callback) {
	...
	if (config.flTraceOnError === undefined) { //12/16/22 by DW
		config.flTraceOnError = true;
		}
	if (config.flDebugOutgoingHeaders === undefined) { //5/31/23 by DW
		config.flDebugOutgoingHeaders = false;
		}
	...
	function handleRequest (httpRequest, httpResponse) {
		function doHttpReturn (code, type, s, headers) { //10/7/16 by DW
			...
			if (utils.getBoolean (config.flDebugOutgoingHeaders)) { //5/31/23 by DW
				console.log ("davehttp: headers == " + utils.jsonStringify (headers));
				}
		...
		dnsGetDomainName (remoteAddress, function (err, domain) {
			...
			function callBackToApp (theRequest) {
				if (callback !== undefined) {
					if (config.flTraceOnError) { //12/16/22 by DW
						try {
							callback (theRequest);
							}
						catch (err) {
