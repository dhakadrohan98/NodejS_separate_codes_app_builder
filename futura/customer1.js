...
async function main(params) {
    ...
    try {
        ...
        if (typeof params.data.value.params !== "undefined") {
            ...
            if(typeof params.data.value.id !== "undefined") {
                responseData["id"] = params.data.value.id;
            }
        }
        else {
        ...
        try {
            ...
            getCustomer(params, paramsRequest.entity_id)
            //Stroing magento customer id for later use
            var customerID = paramsRequest.entity_id;
            var futuraId, id
            ...
            if (futuraId) {
                ...
                if (typeof paramsRequest.futura_search_email == "undefined" || paramsRequest.futura_search_email.status == false) {
                    try {
                        email = paramsRequest.email;
                        searchcustomerdata['integration'] = "Futura"
                        searchcustomerdata['action'] = "Search Customer"
                        var searchPaylod =  await payloadForSearch(email);
                        searchcustomerdata['request'] = searchPaylod;
                        email = searchPaylod.web_search_kde.web_flds_fill.string[1]; //extracting email from web_flds_fill.string array
                        result = await
                        SearchInFutura(params, searchPaylod, email);
            ...
            var updatecustomer, expirydate, givexnumber, rewarddate
            ...
            var getcustomerdata = {}
            // If we find Futura ID then find details of customer from Futura, Updae customer in Futura
            if (id) {
                ...
                if (id && timeouterror == false && getcustomerdata.status == true) {
                    ...
                    if (paramsRequest.dob) {
                        const [datesting, time] = paramsRequest.dob.split(' ');
                    ...
                    var email = paramsRequest.email
                    updatecustomer.address.web_ans_name1 = paramsRequest.firstname
                    updatecustomer.address.web_ans_name2 = paramsRequest.lastname
                    ...
                    if (magentoerp == true || (expirydate && expirydate != "1899-12-29 00:00:00" && expirydate != "1899-12-30 00:00:00" && magentoreward == true)) {
                        ...
                        UpdateCustomerInMagento(params, {"customer": magecustomer}, paramsRequest.entity_id);
                    }
                }
                ...
                responseData['futura_get_customer_detail']['status'] = true;
                responseData['futura_update_customer']['status'] = true;
                responseData['givex_update_customer']['status'] = true;
                responseData['dusk_portal_update_customer']['status'] = true;
            }
        } catch (error) {
            if (typeof responseData['futura_search_email']['status'] == "undefined") {
                responseData['futura_search_email']['status'] = false;
            }
            ...
            if (id && typeof responseData['futura_update_customer']['status'] == "undefined") {
                responseData['futura_update_customer']['status'] = false;
            }
            if (id && typeof responseData['givex_update_customer']['status'] == "undefined") {
                responseData['givex_update_customer']['status'] = false;
            }
            if (id && typeof responseData['dusk_portal_update_customer']['status'] == "undefined") {
                responseData['dusk_portal_update_customer']['status'] = false;
            }
        }
